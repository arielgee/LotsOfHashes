...
// Tiger.cpp : implementation file
//

// Free implementation of the TIGER hash algorithm

// According to Tiger's web page: http://www.cs.technion.ac.il/~biham/Reports/Tiger/
...
const UINT64 tiger_table[4*256] = {
...
#define save_abc \
	...
	#define round(a,b,c,x,mul) \
			c ^= x; \
			a -= t1[(UCHAR)(c)] ^ t2[(UCHAR)(((ULONG)(c))>>16)] ^ t3[(UCHAR)((c)>>32)] ^ t4[(UCHAR)(((ULONG)((c)>>32))>>16)] ; \
			b += t4[(UCHAR)(((ULONG)(c))>>8)] ^ t3[(UCHAR)(((ULONG)(c))>>24)] ^ t2[(UCHAR)(((ULONG)((c)>>32))>>8)] ^ t1[(UCHAR)(((ULONG)((c)>>32))>>24)]; \
			b *= mul;
...
#define tiger_compress_macro(str, state, nPasses) \
{ \
  register UINT64 a, b, c, tmpa; \
  UINT64 aa, bb, cc; \
  register UINT64 x0, x1, x2, x3, x4, x5, x6, x7; \
  UCHAR pass_no;					 \
  UCHAR passes = nPasses;		 \
\
  a = state[0]; \
  b = state[1]; \
  c = state[2]; \
...
\
  ...
  state[2] = c; \
	...
	#define tiger_compress(str, state, nPasses) \
	  tiger_compress_macro(((UINT64*)str), ((UINT64*)state), ((UCHAR)nPasses))

#else /* OPTIMIZE_FOR_ALPHA */

	/* The compress function is a function. Requires smaller cache?    */
	static void tiger_compress(UINT64 *str, UINT64 state[3], UCHAR nPasses)
	{
	  tiger_compress_macro(((UINT64*)str), ((UINT64*)state), ((UCHAR)nPasses));
	}
...
#define EXTRACT_UCHAR(p)  (*(UCHAR *)(p))
#endif

#define STRING2INT64(s) ((((((((((((((UINT64)(EXTRACT_UCHAR(s+7) << 8)    \
			 | EXTRACT_UCHAR(s+6)) << 8)  \
			 | EXTRACT_UCHAR(s+5)) << 8)  \
			 | EXTRACT_UCHAR(s+4)) << 8)  \
			 | EXTRACT_UCHAR(s+3)) << 8)  \
...
/////////////////////////////////////////////////////////////////////
//
void CTiger::Init(tiger_ctx* ctx, UCHAR nPasses)
{
...
/////////////////////////////////////////////////////////////////////
//
void CTiger::Update(tiger_ctx* ctx, UCHAR* buffer, ULONG len)
{
	ULONG left;
	...
	if (ctx->index) {	/* Try to fill partial block */
		...
		if (len < left)
		{
			...
			this->block(ctx, (UCHAR*)ctx->block);
			buffer += left;
			len -= left;
		}
	}
...
/////////////////////////////////////////////////////////////////////
//
void CTiger::Final(tiger_ctx* ctx)
{
	register UINT64 i, j;
	UCHAR temp[TIGER_DATASIZE];
	i = ctx->index;
	...
	if(j>56)
	{
		for(; j<64; j++)
			temp[j] = 0;
		tiger_compress(((UINT64*)temp), ctx->digest, m_nPasses);
		j=0;
	}
	...
	((UINT64*)(&(temp[56])))[0] = (ctx->count << 9) + (ctx->index << 3);
	tiger_compress(((UINT64*)temp), ctx->digest, m_nPasses);
}

/////////////////////////////////////////////////////////////////////
//
void CTiger::Digest128(tiger_ctx* ctx, UCHAR* s)
{
	ULONG i;
...
/////////////////////////////////////////////////////////////////////
//
void CTiger::Digest160(tiger_ctx *ctx, UCHAR* s)
{
	ULONG i;
	if (s==NULL) return;
...
/////////////////////////////////////////////////////////////////////
//
void CTiger::Digest192(tiger_ctx* ctx, UCHAR* s)
{
	ULONG i;

	if (s!=NULL)
	for (i = 0; i < TIGER192_DIGESTLEN; i++) { /* 64 bit and LITTLE ENDIAN -that's cool! */
		s[7] = ctx->digest[i];
		s[6] = 0xff & (ctx->digest[i] >> 8);
		s[5] = 0xff & (ctx->digest[i] >> 16);
		s[4] = 0xff & (ctx->digest[i] >> 24);
...
/////////////////////////////////////////////////////////////////////
//
void CTiger::block(tiger_ctx *ctx, UCHAR* str)
{
	UINT64 temp[TIGER_DATALEN];
#if defined(WORDS_BIGENDIAN)
	ULONG j;
	for(j=0; j<TIGER_DATALEN; j++, str+=8)
		temp[j] = STRING2INT64(str);
	tiger_compress(temp, ctx->digest, m_nPasses);
#else
