...
// Snefru.cpp : implementation file
//
...
static const UINT32 SBOX[R * 256 * 2] = {
...
#define SWAP_ENDIAN(a) \
	((UINT32) \
		(((a & (UINT32) 0x000000ffU) << 24) | \
		 ((a & (UINT32) 0x0000ff00U) << 8)  | \
		 ((a & (UINT32) 0x00ff0000U) >> 8)  | \
		 ((a & (UINT32) 0xff000000U) >> 24))  \
	)

/////////////////////////////////////////////////////////////////////
//
CSnefru::CSnefru(void)
{
}
...
void CSnefru::Init(snefru_ctx* ctx)
{
	ctx->hashlen = 0;
	ctx->index = 0;
...
/////////////////////////////////////////////////////////////////////
//
void CSnefru::Update128(snefru_ctx* ctx, const UINT8 *data, UINT32 length)
{
	this->update(ctx, data, length, SNEFRU128_DATA_SIZE, SNEFRU128_DIGEST_LEN);
}

/////////////////////////////////////////////////////////////////////
//
void CSnefru::Update256(snefru_ctx* ctx, const UINT8 *data, UINT32 length)
{
	this->update(ctx, data, length, SNEFRU256_DATA_SIZE, SNEFRU256_DIGEST_LEN);
}

/////////////////////////////////////////////////////////////////////
// Finalize the hash value calculation by appending the (padded) length field.
void CSnefru::Final128(snefru_ctx* ctx) 
{
	if (ctx->index)
	...
	memset(ctx->hash + SNEFRU128_DIGEST_LEN, 0, SNEFRU_BLOCK_SIZE - SNEFRU128_DIGEST_SIZE - sizeof(ctx->hashlen));
	ctx->hash[SNEFRU_BLOCK_LEN - 2] = ctx->hashlen >> 32;
	ctx->hash[SNEFRU_BLOCK_LEN - 1] = ctx->hashlen & 0xffffffff;

	this->transform(ctx->hash, SNEFRU128_DIGEST_LEN);
}

/////////////////////////////////////////////////////////////////////
// Finalize the hash value calculation by appending the (padded) length field.
void CSnefru::Final256(snefru_ctx *ctx)
{
	if (ctx->index)
	{    /* pad the last data block if partially filled */
	...
	memset(ctx->hash + SNEFRU256_DIGEST_LEN, 0, SNEFRU_BLOCK_SIZE - SNEFRU256_DIGEST_SIZE - sizeof(ctx->hashlen));
	ctx->hash[SNEFRU_BLOCK_LEN - 2] = ctx->hashlen >> 32;
	ctx->hash[SNEFRU_BLOCK_LEN - 1] = ctx->hashlen & 0xffffffff;
  
	this->transform(ctx->hash, SNEFRU256_DIGEST_LEN);
}

/////////////////////////////////////////////////////////////////////
// 
void CSnefru::Digest128(const snefru_ctx* ctx, UINT8* digest)
{
	this->digest(ctx, digest, SNEFRU128_DIGEST_LEN);
}

/////////////////////////////////////////////////////////////////////
// 
void CSnefru::Digest256(const snefru_ctx* ctx, UINT8* digest)
{
	this->digest(ctx, digest, SNEFRU256_DIGEST_LEN);
}

/////////////////////////////////////////////////////////////////////
// Update the hashing state.
void CSnefru::update(snefru_ctx* ctx, const UINT8* data, UINT32 length, UINT32 data_size, UINT32 digest_len)
{
	if (ctx->index)
	{	/* Try to fill partial block */
		UINT32 left = data_size - ctx->index;
		if (length < left)
		{
...
/////////////////////////////////////////////////////////////////////
//
void CSnefru::digest(const snefru_ctx* ctx, UINT8* digest, UINT32 len)
{
	UINT32 i;

	for (i = 0; i < len; i++, digest += 4)
	{
		*(UINT32 *)digest = SWAP_ENDIAN(ctx->hash[i]);
	}
}

/////////////////////////////////////////////////////////////////////
// The core snefru transform.
void CSnefru::transform(UINT32* block, UINT32 len)
{
	const UINT32 st[4] = {16, 8, 16, 24};
	UINT32 isave[SNEFRU256_DIGEST_LEN];
	UINT32 *sbox, x;
	UINT32 i, j;
	const UINT32 *limit;
	...
	for (sbox = (UINT32 *) SBOX; sbox < limit; sbox += 512)
	{
		for (j = 0; j < 4; j++)
		{
...
/////////////////////////////////////////////////////////////////////
//
void CSnefru::processBuffer(snefru_ctx* ctx, int len)
{
	UINT32 i;
	UINT8 *buf = ctx->buffer;

	for(i = len; i < SNEFRU_BLOCK_LEN; i++, buf += 4)
	{
		ctx->hash[i] = SWAP_ENDIAN(*(UINT32 *)buf);
	}
	CSnefru::transform(ctx->hash, len);
}
