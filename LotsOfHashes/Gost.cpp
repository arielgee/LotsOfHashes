...
// Gost.cpp : implementation file
//

/* This implementation was taken from the public domain, according to
 * the author. --nikos
 */
...
static const UINT32 gost_sbox_1[256] = {
...
static const UINT32 gost_sbox_2[256] = {
...
static const UINT32 gost_sbox_3[256] = {
...
static const UINT32 gost_sbox_4[256] = {
...
void CGost::Update(gost_ctx* ctx, const UINT8* buf, UINT32 len)
{
	UINT32 i, j;
	...
	this->bytes(ctx, ctx->partial, 256);

	while ((j + 32) < len) {
		this->bytes(ctx, &buf[j], 256);
		j += 32;
	}
...
void CGost::Digest(gost_ctx* ctx, UINT8* digest)
{
	UINT32 i;
	UINT32 j;
	UINT32 a;
	...
	if (ctx->partial_bytes > 0) {
		...
		this->bytes(ctx, ctx->partial, ctx->partial_bytes << 3);
	}
	...
	this->compress(ctx->hash, ctx->len);
	this->compress(ctx->hash, ctx->sum);
...
void CGost::compress(UINT32* h, UINT32* m)
{
	UINT32 i;
...
void CGost::bytes(gost_ctx* ctx, const UINT8* buf, UINT32 bits)
{
	UINT32 i;
	UINT32 j;
	UINT32 a, c, m[8];
	...
	c = 0;
	for (i = 0; i < 8; i++) {
		a = ((UINT32) buf[j]) | (((UINT32) buf[j + 1]) << 8) | (((UINT32) buf[j + 2]) << 16) | (((UINT32) buf[j + 3]) << 24);
		j += 4;
		m[i] = a;
		c = a + c + ctx->sum[i];
